...
describe('database layer', () => {
  ...
  test('movement_rules table exists', () => {
    const result = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='movement_rules'"
    );
    expect(result).toHaveLength(1);
  });

  test('welfare_standards table exists', () => {
    const result = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='welfare_standards'"
    );
    expect(result).toHaveLength(1);
  });
});
